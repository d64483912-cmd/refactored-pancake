...
export default function SessionPage() {
  ...
  const { messages, stop, status, sendMessage } = useChat({
    ...
    initialMessages: agent ? [{ id: 'initial', role: 'assistant', content: agent.initialMessage || '' }] : [],
  });
  
  // Persist messages and auto-extract every 3 user messages
  useEffect(() => {
    ...
    if (userMessages.length > userMessageCount && userMessages.length % 3 === 0) {
      setUserMessageCount(userMessages.length);
      apiClient.sessions[':sessionId']['extract-context'].$post({
        param: { sessionId }
      }).catch(() => {});
    }
  }, [messages, sessionId, userMessageCount]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center max-w-md">
          ...
          <button onClick={() => navigate('/')} className="px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90">
            Go Home
          </button>
        </div>
      </div>
